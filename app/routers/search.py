...
def highlight_text(text: str, keyword: str) -> str:
    """Highlight keyword matches in text using <mark> tags."""
    if not text:
        return ""
    # Escape regex special chars from keyword
...
def global_search(db: Session, search_term: str, request: Request , skip: int = 0, limit: int = 10):
    ...
    # Helper function to add result in consistent format
    def add_result(model_name, category, url, title_en, title_ar, description_en, description_ar, image=None):
        # Use Arabic title/description if exists, else fallback to English
        title = title_en if title_en else ""
        description = description_en if description_en else ""
        if title_ar and search_term in title_ar:
            title = title_ar
        if description_ar and search_term in description_ar:
            description = description_ar

        # Convert image path to full URL automatically
        full_image_url = build_image_url(request, image)  # handle image centrally
        results.append({
            "model": model_name,
            "category": category,
            "url": url,
            ...
            "image": full_image_url  # <-- now always full URL if image exists
        })

    # --- FAQ ---
    faq_results = db.query(FAQ).filter(
        or_(FAQ.QuestionEn.ilike(f"%{search_term}%"),
    ...
    dataset_results = db.query(DatasetInfo).filter(
        or_(DatasetInfo.Name.ilike(f"%{search_term}%"),
    ...
    metadata_results = db.query(MetadataInfo).filter(
        or_(MetadataInfo.Name.ilike(f"%{search_term}%"),
    ...
    for meta in metadata_results:
        add_result("MetadataInfo", "Metadata", f"/metadata/{meta.MetadataID}",
                   meta.Name, meta.NameAr,
                   meta.description, meta.descriptionAr,
    ...
    news_results = db.query(News).filter(
        or_(News.TitleEn.ilike(f"%{search_term}%"),
    ...
    for news in news_results:
        add_result("News", "News", f"/news/{news.NewsID}",
                   ...
                   news.DescriptionEn, news.DescriptionAr,
    ...
    product_results = db.query(Product).filter(
        or_(Product.NameEn.ilike(f"%{search_term}%"),
    ...
    project_results = db.query(Projects).filter(
        or_(Projects.NameEn.ilike(f"%{search_term}%"),
    ...
    project_detail_results = db.query(ProjectDetails).filter(
        or_(ProjectDetails.ServiceName.ilike(f"%{search_term}%"),
    ...
    for detail in project_detail_results:
        add_result("ProjectDetail", "ProjectDetails", f"/project-details/{detail.ProjectDetailID}",
                   ...
                   detail.ServiceDescription, detail.ServiceDescriptionAr,
                   image=getattr(detail, "ImageUrl", None))

    # --- ManualGuide ---
    manual_guide_results = db.query(ManualGuide).filter(
        or_(ManualGuide.NameEn.ilike(f"%{search_term}%"),
    ...
    for video in video_results:
        add_result("Video", "Videos", f"/videos/{video.VideoID}",
                   ...
                   video.DescriptionEn, video.DescriptionAr,
                   image=getattr(video, "ImagePath", None))  # or ImageUrl if exists
...
def search(
    ...
    try:
        ...
        skip = (page - 1) * limit
        # Pass request to global_search
        search_results = global_search(db, query, request, skip, limit)

        if not search_results:
            return error_response("No results found for your query.", "NOT_FOUND")
        ...
        return success_response("Search results retrieved successfully.", response_data)

    except Exception as e:
        print("Error in search:", str(e))
        return error_response("An error occurred while processing your request.", "INTERNAL_ERROR")
