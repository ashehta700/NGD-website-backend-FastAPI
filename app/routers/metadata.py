...
def create_dataset(
    ...
    if img:
        folder_path = f"app/static/dataset/{new_dataset.DatasetID}"
        ...
        new_dataset.img = f"static/dataset/{new_dataset.DatasetID}/{img.filename}"
        db.commit()
...
def update_dataset(
    ...
    if img:
        folder_path = f"app/static/dataset/{dataset.DatasetID}"
        os.makedirs(folder_path, exist_ok=True)
        ...
        if dataset.img:
            old_path = os.path.join("app", dataset.img)
        ...
        dataset.img = f"static/dataset/{dataset.DatasetID}/{img.filename}"
...
def create_metadata(
    ...
    # handle file upload
    if file:
        folder_path = f"app/static/dataset/{DatasetID}/metadata/"
        os.makedirs(folder_path, exist_ok=True)
        ...
        new_metadata.FilePath = f"static/dataset/{DatasetID}/metadata/{file.filename}"
        db.commit()
...
def update_metadata(
    ...
    if file:
        folder_path = f"app/static/dataset/{metadata.DatasetID}/metadata/"
        os.makedirs(folder_path, exist_ok=True)
        file_path = os.path.join(folder_path, file.filename)

        # delete old file if exists
        if metadata.FilePath:
            old_path = os.path.join("app", metadata.FilePath)
            if os.path.exists(old_path):
                os.remove(old_path)
        ...
        metadata.FilePath = f"static/dataset/{metadata.DatasetID}/metadata/{file.filename}"
